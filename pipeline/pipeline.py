...
def main():
    ...
    if len(sys.argv) != 5:
        print(
            "Incorrect input. Use as: python script.py 'Area Name' area_key path/to/area.geojson path/to/gtfs.zip"
        )
        sys.exit(1)
    ...
    os.makedirs(f"engine/{name}", exist_ok=True)
    ...
    # grid = gpd.read_parquet(f"{data_folder}/grid_complete.parquet")
    ...
    #     f"{data_folder}/h3/grid_adjacency_binary.parquet"
    # ).transform("r")
    ...
    # # Define the command
    # command = "osmium extract -p temp/aoi.geojson temp/england-latest.osm.pbf -o temp/aoi.osm.pbf"

    # try:
    #     subprocess.run(command, shell=True, check=True)
    #     print("OSM extracted successfully.")
    ...
    # ttm_complete.to_parquet("temp/ttm_complete.parquet")
    ...
    # gs_sites = gpd.read_file("temp/greenspace.gpkg", layer="sites").rename(
    #     columns={"id": "id_site"}
    # )
    # gs_entrances = gpd.read_file("temp/greenspace.gpkg", layer="access_points").rename(
    ...
    # gs_acc.to_parquet("temp/acc_greenspace_allmodes_15min.parquet")
    ...
    # ).replace(np.nan, None).to_parquet(f"engine/{name}/empty.parquet")
    ...
    # ).to_parquet(f"engine/{name}/oa_key.parquet")
